...
void ApplicationSolar::initializeTextures(){
  for(int i = 0; i < planet_textures.size(); i++){
    ...
    glTexImage2D(GL_TEXTURE_2D, 0,
      planet_textures[i].pixelDats_.channels,
      planet_textures[i].pixelDats_.width,
      planet_textures[i].pixelDats_.height, 0,
      planet_textures[i].pixelDats_.channels,
      planet_textures[i].pixelDats_.channel_type,
      planet_textures[i].pixelDats_.ptr());
...
void ApplicationSolar::initializePlanets(){
  ...
  solarsystem_planets_.push_back(Planet("Merkur", 1.383f, 3.012f, 1.0f*1.2, {0.7,0.0, 0.2}));
  solarsystem_planets_.push_back(Planet("Venus", 0.950f, 1.177f, 2.723f*1.3, {0.7,0.6, 0.2}));
  solarsystem_planets_.push_back(Planet("Erde", 1.0f, 1.0f, 3.0f*1.4, {0.7,0.0, 0.22}));
  solarsystem_planets_.push_back(Planet("Mars",0.583f, 0.53f, 4.524f*1.5, {0.7,0.2, 0.2}));
  solarsystem_planets_.push_back(Planet("Jupiter", 1.97f, 0.084f, 5.2f*1.6, {0.7,0.5, 0.42}));
  solarsystem_planets_.push_back(Planet("Saturn",1.14f, 0.0339f, 8.54f*1.7, {0.7,0.0, 0.2}));
  solarsystem_planets_.push_back(Planet("Uranus", 0.98f, 0.0119f, 10.19f*1.8, {0.7,0.3, 0.2}));
  solarsystem_planets_.push_back(Planet("Neptun", 1.87f, 0.006f, 12.1f*1.9, {0.7,0.0, 0.52}));
  ...
  GLuint sun_handle =  texture_obj_container[8].handle;
  Geometrynode* sun   =  new Geometrynode("sun", 2.0f, 0.0f, 0.0f, {0.7,0.0, 0.52}, sun_handle);
  ...
  for(int i = 0; i < solarsystem_planets_.size(); ++i){
    Geometrynode* planet = new Geometrynode(solarsystem_planets_[i].name,
                                            ...
                                            texture_obj_container[i].handle);
...
void ApplicationSolar::initializeGeometry() {
  ...
  glBufferData(GL_ARRAY_BUFFER, quad_model.data.size() * sizeof(float), quad_model.data.data(), GL_STATIC_DRAW);
  // activate first attribute on gpu
  glEnableVertexAttribArray(0);
  // first attribute is 3 floats with no offset & stride
  ...
  // configure currently bound array buffer
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, model::INDEX.size * quad_model.indices.size(), quad_model.indices.data(), GL_STATIC_DRAW);
...
void ApplicationSolar::initializeFramebuffer(){
 ...
 glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, initial_resolution.x, initial_resolution.y);

 glActiveTexture(GL_TEXTURE0);
 glGenTextures(1, &frame_buffer_tex_obj.handle);
 ...
 glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, frame_buffer_tex_obj.handle, 0);
